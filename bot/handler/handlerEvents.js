...
function getRole(threadData, senderID) {
        ...
        return adminBot.map(String).includes(sid) ? 2 : adminBox.includes(sid) ? 1 : 0;
}
...
function isBannedOrOnlyAdmin(userData, threadData, senderID, threadID, isGroup, commandName, message, lang) {
        ...
        if (isGroup == true) {
                if (
                        ...
                        && !(threadData.adminIDs || []).map(String).includes(String(senderID))
...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
        return async function (event, message) {
                ...
                if (!userData && !isNaN(senderID)) {
                        try { userData = await usersData.create(senderID); } catch (_) {}
                }

                if (!threadData) {
                        if (global.temp.createThreadDataError.includes(threadID)) {
                                if (isGroup) return;
                                // DM في قائمة الأخطاء — استخدم stub بدلاً من الحذف
                                threadData = {
                                        threadID,
                                ...
                                try {
                                        ...
                                        // For DMs with non-numeric IDs, build a minimal threadData stub
                                        threadData = {
                                                threadID,
                                                threadName: threadID,
                        ...
                        if (
                                ...
                                global.db.receivedTheFirstMessage[threadID] = true;
                                await threadsData.refreshInfo(threadID).catch(() => {});
                        }
                }
                ...
                */
                ...
                async function onStart() {
                        ...
                        if (isBannedOrOnlyAdmin(userData, threadData, senderID, threadID, isGroup, commandName, message, langCode))
                                return;
                        // —————  CHECK GROUP LOCK (bot admins only)  ————— //
                        if (global.GoatBot.lockedThreads?.[threadID]) {
                                const adminBot = global.GoatBot.config.adminBot || [];
                                const isAdmin = adminBot.includes(String(senderID)) || adminBot.includes(senderID);
                                if (!isAdmin) return;
                        }
                        if (!command) {
