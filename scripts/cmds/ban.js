...
module.exports = {
        ...
        onStart: async function ({ message, event, args, threadsData, getLang, usersData, api }) {
                ...
                // FIX: also protect bot admins and super admins from being banned
                const botAdmins = [
                        ...(global.GoatBot.config.adminBot      || []),
                        ...(global.GoatBot.config.superAdminBot || [])
                ].map(String);
                if (botAdmins.includes(String(target)))
                        return message.reply(getLang('cantBanAdmin'));
                // adminIDs may be stored as strings or objects depending on GoatBot version
                const isGroupAdmin = Array.isArray(adminIDs) && adminIDs.some(a =>
                        (typeof a === "object" ? a.adminID : a) == target
                );
                if (isGroupAdmin)
                        return message.reply(getLang('cantBanAdmin'));
                ...
                message.reply(getLang('bannedSuccess', name), () => {
                        // FIX: members is a plain object (key=userID), not an array — use hasOwnProperty
                        const userIsInGroup = members.hasOwnProperty(target) || event.participantIDs?.includes(target);
                        if (userIsInGroup) {
                                const botIsAdmin = Array.isArray(adminIDs) && adminIDs.some(a =>
                                        (typeof a === "object" ? a.adminID : a) == api.getCurrentUserID()
                                );
                                if (botIsAdmin) {
