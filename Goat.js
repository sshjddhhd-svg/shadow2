...
// ─── Node <18 polyfills — MUST be first, before ANY require() ────────────────
(function applyPolyfills() {
        // 1. Web Streams (Node 16 doesn't expose these globally)
        ...
        // 3. File (needed by undici 6.x — absent in Node <18.13)
        if (typeof globalThis.File === "undefined") {
                try {
                        // Node 18.13+ exposes File via buffer module
                        const { File } = require("buffer");
                        if (File) { globalThis.File = File; }
                } catch (_) {}
        }
        if (typeof globalThis.File === "undefined") {
                // Fallback: minimal File class built on top of Blob
                const BaseBlob = globalThis.Blob || class Blob {};
                globalThis.File = class File extends BaseBlob {
                        constructor(chunks, name, opts = {}) {
                                super(chunks, opts);
                                this._name         = name || "";
                                this._lastModified = opts.lastModified != null ? opts.lastModified : Date.now();
                        }
        ...
        // 4. TextEncoder / TextDecoder (Node 14 global gap)
...
global.GoatBot = {
        ...
        lockedThreads: {}, // store locked threads (populated from lockData.json on lock.js load)
